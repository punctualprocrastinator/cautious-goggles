...
\bottomrule
\end{tabular}
\caption{Zero-shot UniRef50 hits, ordered by novelty. $^\ast$Ecp2: experimentally validated fungal effector---cross-kingdom generalisation from a probe trained exclusively on animal and bacterial toxins. $^\dagger$A0A009RXF9 (67 aa); AlphaFold pLDDT = 43.6 (fully disordered), candidate for wet-lab cytotoxicity assay. $^\ddagger$A0A009QBX0; confirmed via AlphaFold EBI (pLDDT = 83.0, 44.6\% residues $>90$). Five organism groups account for 65.3\% of all 248 hits: \textit{C.\ Accumulibacter} (50), \textit{Ruminococcus} (39), \textit{Acinetobacter} incl.\ \textit{baumannii} (26), \textit{Pseudomonas} (24), and \textit{Colletotrichum}/Pezizomycotina (23)---the probe homes in on virulence-rich species, not random proteins.}
\end{table}
